...
- **filtration**: limit `on_time_sync` recalculation to first boot only — periodic HA re-syncs (~15 min) no longer trigger unnecessary schedule recomputes

### :memo: Documentation Changes

- **filtration-autonome**: clarify scheduler cadence vs. recalculation triggers; correct Courbe duration table (values were wrong) and document high-temperature continuous-filtration behaviour
